...
public class GarbageCollectedMemoryPool extends SimpleMemoryPool implements AutoCloseable {
	...
	public GarbageCollectedMemoryPool(long sizeBytes, int maxSingleAllocationSize) {
		super(sizeBytes, maxSingleAllocationSize);
		this.alive = true;
		this.gcListenerThread = new Thread(gcListener, "memory pool GC listener");
		this.gcListenerThread.setDaemon(true); //so we don't need to worry about shutdown
	...
	protected void bufferToBeReleased(ByteBuffer justReleased) {
		...
		if (metadata == null)
			//it's impossible for the buffer to have already been GC'ed (because we have a hard ref to it
